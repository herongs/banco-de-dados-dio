...
-- 7) Consulta titulo livro e nome estilo, dos livros do estilo Romance;
...
select l.titulo, e.nome, ed.nome from livro as l
join livro_estilo as le on l.id_livro = le.id_livro
join estilo as e on e.id_estilo = le.id_estilo
join editora as ed on ed.id_editora = l.id_editora
where ed.nome = 'Globo';
...
select ed.nome from editora as ed
...
select e.nome from estilo as e
left join livro_estilo as le on e.id_estilo = le.id_estilo
where le.id_estilo is null;

-- 21)Consultar os nomes dos autores e quantos livros cada autor possui; 


select a.nome, count(a.id_autor) from livro as l
join livro_autor as la on la.id_livro = l.id_livro
join autor as a on a.id_autor = la.id_autor
group by a.nome;
...
select ed.nome, count(ed.id_editora) from livro as l 
join editora as ed on ed.id_editora = l.id_editora
group by ed.nome;

-- 23)Consultar os nomes das estilo e quantos livros cada estilo possui;

select e.nome, count(le.id_livro) from estilo as e
inner join livro_estilo as le on le.id_estilo = e.id_estilo
group by e.nome;

-- 24)Consultar quantos livros a editora CCCC possui; 

select ed.nome, count(l.id_livro) from editora as ed 
inner join livro as l on ed.id_editora = l.id_editora
where ed.nome like 'cccc';

-- 25)Consultar quantos livros o autor J R R Tolkien possui;

select a.nome,count(la.id_livro) from autor as a
